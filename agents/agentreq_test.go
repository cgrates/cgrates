...
func TestAgReqSetFields(t *testing.T) {
	...
	agReq.CGRRequest.Set(utils.PathItems{{Field: utils.CGRID}},
		utils.NewNMData(utils.Sha1("dsafdsaf", time.Date(2013, 11, 7, 8, 42, 26, 0, time.UTC).String())))
	agReq.CGRRequest.Set(utils.PathItems{{Field: utils.ToR}}, utils.NewNMData(utils.VOICE))
	agReq.CGRRequest.Set(utils.PathItems{{Field: utils.Account}}, utils.NewNMData("1001"))
	agReq.CGRRequest.Set(utils.PathItems{{Field: utils.Destination}}, utils.NewNMData("1002"))
	agReq.CGRRequest.Set(utils.PathItems{{Field: utils.AnswerTime}},
		utils.NewNMData(time.Date(2013, 12, 30, 15, 0, 1, 0, time.UTC)))
	agReq.CGRRequest.Set(utils.PathItems{{Field: utils.RequestType}}, utils.NewNMData(utils.META_PREPAID))
	agReq.CGRRequest.Set(utils.PathItems{{Field: utils.Usage}}, utils.NewNMData(time.Duration(3*time.Minute)))
	...
	eMp.Set(utils.PathItems{{Field: utils.Destination}}, &utils.NMSlice{
		&config.NMItem{Data: "1002", Path: []string{utils.Destination},
			Config: tplFlds[2]}})
	eMp.Set(utils.PathItems{{Field: "RequestedUsage"}}, &utils.NMSlice{
		&config.NMItem{Data: "180", Path: []string{"RequestedUsage"},
			Config: tplFlds[3]}})
	eMp.Set(utils.PathItems{{Field: "PaypalAccount"}}, &utils.NMSlice{
		&config.NMItem{Data: "cgrates@paypal.com", Path: []string{"PaypalAccount"},
			Config: tplFlds[6]}})
	eMp.Set(utils.PathItems{{Field: "MaxUsage"}}, &utils.NMSlice{
...
func TestAgentRequestSetFields(t *testing.T) {
	...
	input = []*config.FCTemplate{
		&config.FCTemplate{
			Path:  utils.MetaVars + utils.NestingSep + "Account",
	...
	input = []*config.FCTemplate{
		&config.FCTemplate{
			Path:  utils.MetaCgreq + utils.NestingSep + "Account",
	...
	input = []*config.FCTemplate{
		&config.FCTemplate{
			Path:  utils.MetaCgrep + utils.NestingSep + "Account",
	...
	input = []*config.FCTemplate{
		&config.FCTemplate{
			Path:  utils.MetaRep + utils.NestingSep + "Account",
	...
	input = []*config.FCTemplate{
		&config.FCTemplate{
			Path:  utils.MetaDiamreq + utils.NestingSep + "Account",
	...
	input = []*config.FCTemplate{
		&config.FCTemplate{
			Path:  utils.MetaVars + utils.NestingSep + "AccountID",
	...
	input = []*config.FCTemplate{
		&config.FCTemplate{
			Path:    utils.MetaVars + utils.NestingSep + "Name",
	...
	input = []*config.FCTemplate{
		&config.FCTemplate{
			Path:  utils.MetaVars + utils.NestingSep + "Test",
	...
	input = []*config.FCTemplate{
		&config.FCTemplate{
			Path:  utils.MetaVars + utils.NestingSep + "Account4",
	...
	input = []*config.FCTemplate{
		&config.FCTemplate{
			Path:  utils.MetaVars + utils.NestingSep + "Account5",
...
func TestAgReqMaxCost(t *testing.T) {
	...
	agReq := NewAgentRequest(nil, nil, nil, nil, nil, "cgrates.org", "", filterS, nil, nil)
	// populate request, emulating the way will be done in HTTPAgent
	agReq.CGRRequest.Set(utils.PathItems{{Field: utils.CapMaxUsage}}, utils.NewNMData("120s"))
...
func TestAgReqMetaExponent(t *testing.T) {
	...
	filterS := engine.NewFilterS(cfg, nil, dm)
	agReq := NewAgentRequest(nil, nil, nil, nil, nil, "cgrates.org", "", filterS, nil, nil)
	agReq.CGRRequest.Set(utils.PathItems{{Field: "Value"}}, utils.NewNMData("2"))
	agReq.CGRRequest.Set(utils.PathItems{{Field: "Exponent"}}, utils.NewNMData("2"))
...
func TestAgReqFieldAsInterface(t *testing.T) {
	...
	agReq.CGRRequest.Set(utils.PathItems{{Field: utils.Usage}}, &utils.NMSlice{&config.NMItem{Data: 3 * time.Minute}})
	agReq.CGRRequest.Set(utils.PathItems{{Field: utils.ToR}}, &utils.NMSlice{&config.NMItem{Data: utils.VOICE}})
	agReq.CGRRequest.Set(utils.PathItems{{Field: utils.Account}}, utils.NewNMData("1001"))
	agReq.CGRRequest.Set(utils.PathItems{{Field: utils.Destination}}, utils.NewNMData("1002"))
	...
	agReq.tmp = utils.NavigableMap2{utils.Destination: utils.NewNMData("1001")}
	...
	agReq.diamreq.Set(utils.PathItems{{Field: utils.Destination}}, utils.NewNMData("1001"))
...
func TestAgReqNewARWithCGRRplyAndRply(t *testing.T) {
	...
	rply.Set(utils.PathItems{{Field: "FirstLevel"}, {Field: "SecondLevel"}, {Field: "Fld1"}}, utils.NewNMData("Val1"))
	...
	eMp := utils.NewOrderedNavigableMap()
	eMp.Set(utils.PathItems{{Field: "Fld1"}}, &utils.NMSlice{
		&config.NMItem{Data: "Val1", Path: []string{"Fld1"},
			Config: tplFlds[0]}})
	eMp.Set(utils.PathItems{{Field: "Fld2"}}, &utils.NMSlice{
