...
func (sma *AsteriskAgent) handleStasisStart(ev *SMAsteriskEvent) {
	...
	for key, val := range ev.cachedFields {
		if key == utils.CGRReqType || key == utils.CGRFlags || key == utils.CGRRoutes {
...
func (sma *AsteriskAgent) handleChannelDestroyed(ev *SMAsteriskEvent) {
	...
	if !hasIt {
		...
		if err := sma.connMgr.Call(sma.ctx, sma.cgrCfg.AsteriskAgentCfg().SessionSConns, utils.SessionSv1GetActiveSessions, &utils.SessionFilter{}, &reply); err != nil && err.Error() != utils.ErrNotFound.Error() {
			utils.Logger.Warning(fmt.Sprintf("<AsteriskAgent> Error getting active sessions: <%v>", err))
			return
		}
		var hasChID bool             // return if ChannelID is not found as OriginID in active sessions
		for _, sess := range reply { // make sure the asterisk session has a correlating active cgrates session
			if sess.OriginID == chID {
