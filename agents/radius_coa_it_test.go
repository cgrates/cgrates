...
// testing the same as above but using dynamic filter on *alter_sessions to alter all sessions (limited to 1) which match the account in the fields of whatever calles the AlterSession action
func TestRadiusCoADisconnectWithDynamicFilter(t *testing.T) {
	switch *utils.DBType {
	case utils.MetaInternal:
	case utils.MetaMySQL, utils.MetaMongo, utils.MetaPostgres:
		t.SkipNow()
