...
func (rplSv1 *ReplicatorSv1) SetIPProfile(ctx *context.Context, rs *engine.IPProfileWithAPIOpts, reply *string) (err error) {
	if err = rplSv1.dm.DataDB().SetIPProfileDrv(rs.IPProfile); err != nil {
		return
	}
	// delay if needed before cache call
	if rplSv1.v1.Config.GeneralCfg().CachingDelay != 0 {
	...
	if err = rplSv1.v1.CallCache(utils.IfaceAsString(rs.APIOpts[utils.CacheOpt]),
		rs.Tenant, utils.CacheIPProfiles, rs.TenantID(), utils.EmptyString, &rs.FilterIDs, nil, rs.APIOpts); err != nil {
		return
	}
	*reply = utils.OK
	return
...
func (rplSv1 *ReplicatorSv1) SetIP(ctx *context.Context, rs *engine.IPWithAPIOpts, reply *string) (err error) {
	if err = rplSv1.dm.DataDB().SetIPDrv(rs.IP); err != nil {
		return
	}
	if err = rplSv1.v1.CallCache(utils.IfaceAsString(rs.APIOpts[utils.CacheOpt]),
		rs.Tenant, utils.CacheIPs, rs.TenantID(), utils.EmptyString, nil, nil, rs.APIOpts); err != nil {
		return
	}
	*reply = utils.OK
	return
