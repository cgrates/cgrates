...
func (self *ApierV2) LoadRatingProfile(attrs AttrLoadRatingProfile, reply *string) error {
	...
	if err := self.DataManager.DataDB().CacheDataFromDB(utils.RATING_PROFILE_PREFIX, []string{attrs.RatingProfileId}, true); err != nil {
		return utils.NewErrServerError(err)
	}
	*reply = v1.OK
	return nil
...
func (self *ApierV2) LoadDerivedChargers(attrs AttrLoadDerivedChargers, reply *string) error {
	...
	if err := self.DataManager.DataDB().CacheDataFromDB(utils.DERIVEDCHARGERS_PREFIX, []string{attrs.DerivedChargersId}, true); err != nil {
		return utils.NewErrServerError(err)
	}
	*reply = v1.OK
	return nil
...
func (self *ApierV2) LoadTariffPlanFromFolder(attrs utils.AttrLoadTpFromFolder, reply *utils.LoadInstance) error {
	...
	loader := engine.NewTpReader(self.DataManager.DataDB(), engine.NewFileCSVStorage(utils.CSV_SEP,
...
func (self *ApierV2) SetActions(attrs utils.AttrSetActions, reply *string) error {
	...
	if !attrs.Overwrite {
		if exists, err := self.DataManager.DataDB().HasData(utils.ACTION_PREFIX, attrs.ActionsId); err != nil {
			return utils.NewErrServerError(err)
		} else if exists {
