...
func TestReplicatorSetIndexesClear(t *testing.T) {
	...
	if err := dataDB.SetIndexesDrv(context.Background(), idxItmType, tntCtx, map[string]utils.StringSet{
		"A": {"old": {}},
		"C": {"old": {}},
	...
	args := &utils.SetIndexesArg{
		...
		Clear:   true,
		Tenant:  "cgrates.org",
		APIOpts: map[string]any{utils.MetaCache: utils.MetaLoad},
	}
	var reply string
	...
	got, err := dataDB.GetIndexesDrv(context.Background(), idxItmType, tntCtx, utils.NonTransactional)
	if err != nil {
		t.Fatal(err)
	}
...
func TestReplayGroupedIndexPatch(t *testing.T) {
	...
	cacheS := engine.NewCacheS(cfg, nil, nil, nil, locker)
	connMgr.SetCache(cacheS)
	connMgr.AddInternalConn(connID, utils.ReplicatorSv1, connector)
	...
	api := NewAdminSv1(cfg, dm, connMgr, engine.NewFilterS(cfg, connMgr, dm), locker)
	var reply string
	if err := api.ReplayFailedReplications(context.Background(), ReplayFailedReplicationsArgs{
