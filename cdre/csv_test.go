...
func TestCsvCdrWriter(t *testing.T) {
	...
	cdre, err := NewCdrExporter([]*engine.StoredCdr{storedCdr1}, logDb, cfg.CdreProfiles["*default"], utils.CSV, ',', "firstexport", 0.0, 0.0, 0.0, 0, 4,
...
func TestAlternativeFieldSeparator(t *testing.T) {
	...
	cdre, err := NewCdrExporter([]*engine.StoredCdr{storedCdr1}, logDb, cfg.CdreProfiles["*default"], utils.CSV, '|', "firstexport", 0.0, 0.0, 0.0, 0, 4, cfg.RoundingDecimals, "", 0, cfg.HttpSkipTlsVerify)
	if err != nil {
		t.Error("Unexpected error received: ", err)
	}
	csvWriter := csv.NewWriter(writer)
