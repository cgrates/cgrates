...
func TestCdreCfgClone(t *testing.T) {
	cgrIDRsrs := NewRSRParsersMustCompile("cgrid", utils.INFIELD_SEP)
	runIDRsrs := NewRSRParsersMustCompile("runid", utils.INFIELD_SEP)
...
"cdre": {
	...
	val, err := NewRSRParsers("~*req.CGRID", utils.INFIELD_SEP)
	if err != nil {
		t.Error(err)
	}
	expected = CdreCfg{
