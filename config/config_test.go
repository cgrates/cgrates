		...
		Readers: []*EventReaderCfg{
			{
				...
				Opts: &EventReaderOpts{
					...
					NATSSubject:         utils.StringPointer("cgratesURs"),
				},
			},
		},
...
func TestV1GetConfigSectionTemplates(t *testing.T) {
	...
	if err := cfgCgr.V1GetConfig(context.Background(), &SectionWithAPIOpts{Sections: []string{TemplatesJSON}}, &reply); err != nil {
		...
		mp[utils.MetaUrLog] = []map[string]any{}
...
func TestV1GetConfigAsJSONFKamailioAgent(t *testing.T) {
	var reply string
	expected := `{"kamailioAgent":{"conns":{"*sessions":[{"filterIDs":null,"tenant":"","connIDs":["*birpc_internal"]}]},"enabled":false,"evapiConns":[{"address":"127.0.0.1:8448","alias":"","maxReconnectInterval":"0s","reconnects":5}],"requestProcessors":[],"timezone":""}}`
	cfgCgr := NewDefaultCGRConfig()
