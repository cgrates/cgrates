...
func TestV1GetConfigStorDB(t *testing.T) {
	...
	expected := map[string]any{
		...
		utils.OptsCfg: map[string]any{
			utils.InternalDBDumpIntervalCfg:    0 * time.Nanosecond,
			...
			utils.InternalDBStartTimeoutCfg:    5 * time.Minute,
			utils.InternalDBRewriteIntervalCfg: 0 * time.Nanosecond,
