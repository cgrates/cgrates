...
"db": {
	...
	} else if cgrCfg.DbCfg().DBConns[utils.MetaDefault].Port != "" {
		t.Errorf("Expected: %+v, received: %+v", cgrCfg.DbCfg().DBConns[utils.MetaDefault].Port, "6379")
	}
	jsnCfg = `
{
"db": {
	...
	if cgrCfg, err := NewCGRConfigFromJSONStringWithDefaults(jsnCfg); err != nil {
		...
		t.Errorf("Expected: %+v, received: %+v", "6379", cgrCfg.DbCfg().DBConns[utils.MetaDefault].Port)
	}
}
