...
func TestEventExporterSameID(t *testing.T) {
	expectedEEsCfg := &EEsCfg{
		Enabled: true,
		Conns: map[string][]*DynamicConns{
			utils.MetaAttributes: {{ConnIDs: []string{"conn1"}}},
		},
		Cache: map[string]*CacheParamCfg{
			utils.MetaFileCSV: {
				Limit:     -1,
...
func TestEEsCfgloadFromJsonCfgCase1(t *testing.T) {
	jsonCfg := &EEsJsonCfg{
		...
		Conns: map[string][]*DynamicConns{
			utils.MetaAttributes: {{ConnIDs: []string{"*conn1", "*conn2"}}},
		},
		Cache: map[string]*CacheParamJsonCfg{
			utils.MetaFileCSV: {
...
func TestEEsCfgAsMapInterface(t *testing.T) {
	...
	eMap := map[string]any{
		...
		utils.ConnsCfg: map[string][]*DynamicConns{
			utils.MetaAttributes: {{ConnIDs: []string{utils.MetaInternal, "*conn2"}}},
		},
...
func TestDiffEventExporterJsonCfg(t *testing.T) {
	...
	v1 := &EventExporterCfg{
		...
		Blocker: true,
		Conns: map[string][]*DynamicConns{
			utils.MetaEFs: {{ConnIDs: []string{"v1 efs test"}}},
		},
	}
	...
	v2 := &EventExporterCfg{
		...
		Blocker: false,
		Conns: map[string][]*DynamicConns{
			utils.MetaEFs: {{ConnIDs: []string{"efs test"}}},
		},
	}
...
func TestDiffEventExporterOptsJsonCfg(t *testing.T) {
	...
	v2 := &EventExporterOpts{
		...
		ElsOpType:                utils.StringPointer("opType"),
		...
		SQLTableName:             utils.StringPointer("urs"),
		...
		KafkaLinger:              utils.DurationPointer(10 * time.Millisecond),
		AMQPRoutingKey:           utils.StringPointer("routingKey"),
		AMQPQueueID:              utils.StringPointer("queueID"),
		AMQPExchange:             utils.StringPointer("amqpExchange"),
		AMQPExchangeType:         utils.StringPointer("amqpExchangeType"),
	...
	exp := &EventExporterOptsJson{
		...
		KafkaCAPath:              utils.StringPointer("kafkaCAPath"),
		KafkaSkipTLSVerify:       utils.BoolPointer(false),
		KafkaLinger:              utils.StringPointer("10ms"),
		AMQPRoutingKey:           utils.StringPointer("routingKey"),
		AMQPQueueID:              utils.StringPointer("queueID"),
		AMQPExchange:             utils.StringPointer("amqpExchange"),
		AMQPExchangeType:         utils.StringPointer("amqpExchangeType"),
...
func TestEventExporterOptsClone(t *testing.T) {
	eeOpts := &EventExporterOpts{
		...
		ElsRouting:                  utils.StringPointer("routing"),
		ElsTimeout:                  utils.DurationPointer(2 * time.Second),
		ElsCAPath:                   utils.StringPointer("elsCAPath"),
		ElsWaitForActiveShards:      utils.StringPointer("wfas"),
		ElsDiscoverNodesOnStart:     utils.BoolPointer(false),
		ElsDiscoverNodeInterval:     utils.DurationPointer(1 * time.Second),
		ElsAPIKey:                   utils.StringPointer("apikey"),
		ElsCertificateFingerprint:   utils.StringPointer("test"),
		ElsServiceToken:             utils.StringPointer("elsTokenTest"),
		ElsUsername:                 utils.StringPointer("username"),
		ElsPassword:                 utils.StringPointer("password"),
		ElsEnableDebugLogger:        utils.BoolPointer(false),
		ElsLogger:                   utils.StringPointer("logger"),
		ElsCompressRequestBody:      utils.BoolPointer(false),
		ElsCompressRequestBodyLevel: utils.IntPointer(5),
		ElsRetryOnStatus:            &[]int{5},
		ElsMaxRetries:               utils.IntPointer(5),
		ElsDisableRetry:             utils.BoolPointer(false),
		SQLMaxIdleConns:             utils.IntPointer(5),
		SQLMaxOpenConns:             utils.IntPointer(10),
		SQLConnMaxLifetime:          utils.DurationPointer(2 * time.Second),
		SQLTableName:                utils.StringPointer("urs"),
		SQLDBName:                   utils.StringPointer("cgrates"),
		SQLUpdateIndexedFields:      utils.SliceStringPointer([]string{"id"}),
		PgSSLMode:                   utils.StringPointer("sslm"),
		KafkaTopic:                  utils.StringPointer("topic1"),
		KafkaLinger:                 utils.DurationPointer(10 * time.Millisecond),
		KafkaDeliveryTimeout:        utils.DurationPointer(30 * time.Second),
...
func TestEEsAsMapInterface(t *testing.T) {
	...
	exp := map[string]any{
		"opts": map[string]any{
			...
			"elsCompressRequestBodyLevel": 5,
			"elsDisableRetry":             false,
			"elsDiscoverNodeInterval":     1000000000,
			"elsDiscoverNodesOnStart":     false,
			"elsEnableDebugLogger":        false,
			"elsLogger":                   "logger",
			"elsMaxRetries":               5,
			"elsPassword":                 "password",
			"elsRetryOnStatus":            []int{5},
			"elsServiceToken":             "elsTokenTest",
			"elsUsername":                 "username",
			"kafkaTopic":                  "topic1",
	...
	if !reflect.DeepEqual(utils.ToJSON(exp[utils.OptsCfg]), utils.ToJSON(rcv[utils.OptsCfg])) {
