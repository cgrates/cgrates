...
func TestStoreDbCfgloadFromJsonCfgCase1(t *testing.T) {
	...
	expected := &StorDbCfg{
		...
		Opts: &StorDBOpts{
			...
			InternalDBDumpInterval:    0 * time.Second,
			InternalDBRewriteInterval: 0 * time.Second,
...
func TestStorDbCfgAsMapInterface(t *testing.T) {
	...
	eMap := map[string]any{
		...
		utils.OptsCfg: map[string]any{
			utils.InternalDBDumpIntervalCfg:    0 * time.Second,
			...
			utils.InternalDBRewriteIntervalCfg: 0 * time.Second,
