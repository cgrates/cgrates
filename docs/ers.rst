			...
			"fields":[					// mapping definition between line index in the file and CGRateS field 
				{
					...
					"type": "*variable",
					"value": "~*req.0",		// take the content from line index 0
...
runDelay
	...
	**0**
		Disables automatic reading; the read is triggered externally (ie. via API).

	**-1**
		Watches the source for changes indefinitely (ie. Linux inotify, for file-based readers).
