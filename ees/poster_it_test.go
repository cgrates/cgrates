...
func TestSQSPoster(t *testing.T) {
	...
	if err := ExportWithAttempts(context.Background(), pstr, []byte(body), "", nil, "cgrates.org", nil); err != nil {
		t.Fatal(err)
	}
...
func TestS3Poster(t *testing.T) {
	...
	if err := ExportWithAttempts(context.Background(), pstr, []byte(body), key, nil, "cgrates.org", nil); err != nil {
		t.Fatal(err)
	}
	key += ".json"
	var sess *session.Session
