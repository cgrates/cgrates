...
func (acc *Account) debitMonetaryBalance(cd *CallDescriptor, count bool, dryRun bool, goNegative bool, fltrS *FilterS, cost float64) (cc *CallCost, err error) {
	usefulMoneyBalances := acc.getAlldBalancesForPrefix(cd.Destination, cd.Category, utils.MetaMonetary, cd.TimeStart)
	// initValues map[UUID]float64 and pass them to publish updating initial value
	...
	for moneyBalanceChecker {
		...
		moneyBalanceChecker = false
		for _, balance := range usefulMoneyBalances {
			partCC, debitErr := balance.debitFromCost(cd, balance.account,
				usefulMoneyBalances, count, dryRun, len(cc.Timespans) == 0, fltrS, cost)
			if debitErr != nil {
				return nil, debitErr
			}
			if partCC != nil {
