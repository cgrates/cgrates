...
func (fS *FilterS) Pass(tenant string, filterIDs []string,
	...
	dDP := newDynamicDP(fS.cfg.FilterSCfg().ResourceSConns, fS.cfg.FilterSCfg().StatSConns,
		fS.cfg.FilterSCfg().ApierSConns, fS.cfg.FilterSCfg().TrendSConns, fS.cfg.FilterSCfg().RankingSConns, tenant, ev)
...
func (fltr *Filter) Clone() *Filter {
	...
	if fltr.ActivationInterval != nil {
		clone.ActivationInterval = fltr.ActivationInterval.Clone()
	}
	return clone
}
...
func (fltr *FilterRule) Clone() *FilterRule {
	...
	if fltr.rsrValues != nil {
		clone.rsrValues = make(config.RSRParsers, len(fltr.rsrValues))
...
func (fltr *FilterRule) FilterToSQLQuery() (conditions []string) {
	...
	jsonExpr := restOfItems // plain column when no JSON nesting
	if firstItem != "" {
		jsonExpr = fmt.Sprintf("JSON_VALUE(%s, '$.%s')", firstItem, restOfItems)
		if strings.HasPrefix(restOfItems, `"*`) {
			jsonExpr = fmt.Sprintf("JSON_UNQUOTE(%s)", jsonExpr)
		}
	}

	if len(fltr.Values) == 0 {
		// MariaDB MDEV-37428: JSON_VALUE returns NULL for empty string
		// properties in 10.11.14 (Ubuntu 24.04 / Debian 12). Use
		// JSON_EXTRACT instead which returns "" correctly. Fixed in
		// 10.11.15, 11.4.9, 11.8.4, 12.1.2.
		expr := restOfItems
		if firstItem != "" {
			expr = fmt.Sprintf("JSON_UNQUOTE(JSON_EXTRACT(%s, '$.%s'))", firstItem, restOfItems)
		}
		switch fltr.Type {
