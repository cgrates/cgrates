...
func TestFilterToSQLQuery(t *testing.T) {
	tests := []struct {
		...
		{"MetaGreaterThan with values", FilterRule{Type: utils.MetaGreaterThan, Element: "~*req.answer_time", Values: []string{"NOW() - INTERVAL 7 DAY"}}, []string{"answer_time > NOW() - INTERVAL 7 DAY"}},
