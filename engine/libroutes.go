...
type RoutesSorter interface {
	SortRoutes(string, map[string]*Route, *CGREvent, *optsGetRoutes) (*SortedRoutes, error)
}
...
func (ssd RouteSortDispatcher) SortRoutes(prflID, strategy string,
	suppls map[string]*Route, suplEv *CGREvent, extraOpts *optsGetRoutes) (sortedRoutes *SortedRoutes, err error) {
	sd, has := ssd[strategy]
	if !has {
