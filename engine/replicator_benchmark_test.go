...
func BenchmarkReplicatorImmediate(b *testing.B) {
	...
	b.Run("FailedDirDisabled", func(b *testing.B) {
		...
		for b.Loop() {
			if err := r.replicate(context.Background(), objType, call.objID, call.method, call.args, item); err != nil {
				b.Fatal(err)
			}
		}
	})
...
func BenchmarkReplicatorFailedWrite(b *testing.B) {
	...
	objType := utils.CacheInstanceToPrefix[utils.CacheAttributeProfiles]
	for b.Loop() {
		if err := r.replicate(context.Background(), objType, call.objID, call.method, call.args, item); !errors.Is(err, wantErr) {
...
func BenchmarkIndexReplication(b *testing.B) {
	...
	for _, fields := range []int{1, 16, 256} {
		...
		b.Run("FailedRetry/"+name, func(b *testing.B) {
			...
			b.ReportAllocs()
			for b.Loop() {
				if err := r.replicateIndexes(context.Background(), objType, tntCtx, args); !errors.Is(err, wantErr) {
...
func BenchmarkReplicationTaskWriteToFile(b *testing.B) {
	cm := setupReplicator(b, "", &mockConnector{})
	path := filepath.Join(b.TempDir(), "replication.gob")
	task := newReplicationTask()
	locker := NewLocker(cm.cfg)
	for b.Loop() {
		if err := task.WriteToFile(context.Background(), path, locker); err != nil {
			b.Fatal(err)
		}
	}
}
...
func BenchmarkNewReplicationTaskFromFile(b *testing.B) {
	...
	for b.Loop() {
		...
		b.StartTimer()
		if _, err := NewReplicationTaskFromFile(context.Background(), path, locker); err != nil {
			b.Fatal(err)
		}
	}
}
