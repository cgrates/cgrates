...
func newOptsGetRoutes(ctx *context.Context, ev *utils.CGREvent, fS *FilterS, cfgOpts *config.RoutesOpts) (opts *optsGetRoutes, err error) {
	...
	if ignoreErrors, err = GetBoolOpts(ctx, ev.Tenant, evNM, nil, fS, cfgOpts.IgnoreErrors,
	...
	var limit *int
	if limit, err = GetIntPointerOpts(ctx, ev.Tenant, evNM, nil, fS, cfgOpts.Limit,
	...
	var offset *int
	if offset, err = GetIntPointerOpts(ctx, ev.Tenant, evNM, nil, fS, cfgOpts.Offset,
	...
	var maxItems *int
	if maxItems, err = GetIntPointerOpts(ctx, ev.Tenant, evNM, nil, fS, cfgOpts.MaxItems,
	...
	var maxCost any
	if maxCost, err = GetInterfaceOpts(ctx, ev.Tenant, evNM, nil, fS, cfgOpts.MaxCost, config.RoutesMaxCostDftOpt,
...
func (rpS *RouteS) V1GetRoutes(ctx *context.Context, args *utils.CGREvent, reply *SortedRoutesList) (err error) {
	...
	if len(rpS.cfg.RouteSCfg().AttributeSConns) != 0 {
		...
		var context string
		if context, err = GetStringOpts(ctx, tnt, args.AsDataProvider(), nil, rpS.fltrS, rpS.cfg.RouteSCfg().Opts.Context,
...
func (rpS *RouteS) sortedRoutesForEvent(ctx *context.Context, tnt string, args *utils.CGREvent) (sortedRoutes SortedRoutesList, err error) {
	...
	var prfCountOpt *int
	if prfCountOpt, err = GetIntPointerOpts(ctx, tnt, args.AsDataProvider(), nil, rpS.fltrS, rpS.cfg.RouteSCfg().Opts.ProfileCount,
