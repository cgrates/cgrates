...
func (ms *MongoStorage) GetKeysForPrefix(prefix string) (keys []string, err error) {
	...
	err = ms.query(func(sctx mongo.SessionContext) error {
		...
		case utils.IPProfilesPrefix:
			keys, qryErr = ms.getAllKeysMatchingTenantID(sctx, ColRsP, utils.IPProfilesPrefix, subject, tntID)
		case utils.IPsPrefix:
			keys, qryErr = ms.getAllKeysMatchingTenantID(sctx, ColRes, utils.IPsPrefix, subject, tntID)
