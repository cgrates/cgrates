...
func (rs *RedisStorage) SetCDR(_ *context.Context, cdr *utils.CGREvent, allowUpdate bool) error {
	...
	cdrMap := make(map[string][]byte) // hash map containing the CDR
	// Marshal the CDR elements
	var err error
	cdrMap[utils.Tenant], err = rs.ms.Marshal(cdr.Tenant)
	if err != nil {
		return err
	}
	cdrMap[utils.ID], err = rs.ms.Marshal(cdr.ID)
	if err != nil {
		return err
	}
	cdrMap[utils.Event], err = rs.ms.Marshal(cdr.Event)
	if err != nil {
		return err
	}
	cdrMap[utils.APIOpts], err = rs.ms.Marshal(cdr.APIOpts)
	if err != nil {
		return err
	}

	// Store the CDR in Redis
	if err := rs.FlatCmd(nil, redisHSET, utils.CDRsPrefix+cdrID, cdrMap); err != nil {
		return err
	}
...
func (rs *RedisStorage) GetCDRs(ctx *context.Context, qryFltr []*Filter, opts map[string]any) (cdrs []*utils.CDR, err error) {
	...
	for keySlice, fltrSlice := range pairFltrs {
		...
		for _, id := range fltrSlice {
			var ids []string
			if err := rs.Cmd(&ids, redisSMEMBERS, utils.CDRsIndexes+utils.ConcatenatedKey(keySlice, id)); err != nil {
				return nil, err
			}
	...
	for key := range cdrMpIDs {
		cdrBytes := make(map[string][]byte)
		// key includes "cdr_" prefix
		if err := rs.Cmd(&cdrBytes, redisHGETALL, key); err != nil {
			return nil, err
		}
		cgrEv := new(utils.CGREvent)
		for key, fieldBytes := range cdrBytes {
			switch key {
			case utils.Tenant:
				if err := rs.ms.Unmarshal(fieldBytes, &cgrEv.Tenant); err != nil {
					return nil, err
				}
			case utils.ID:
				if err := rs.ms.Unmarshal(fieldBytes, &cgrEv.ID); err != nil {
					return nil, err
				}
			case utils.Event:
				if err := rs.ms.Unmarshal(fieldBytes, &cgrEv.Event); err != nil {
					return nil, err
				}
			case utils.APIOpts:
				if err := rs.ms.Unmarshal(fieldBytes, &cgrEv.APIOpts); err != nil {
					return nil, err
				}
			}
		}
...
func (rs *RedisStorage) RemoveCDRs(ctx *context.Context, qryFltr []*Filter) (err error) {
	...
	for key := range cdrMpIDs {
		...
		cgrEv := new(utils.CGREvent)
		var cdrTenant string
		for key, fieldBytes := range cdrBytes {
			switch key { // Tenant and ID are not considered for this
			case utils.Event:
				if err := rs.ms.Unmarshal(fieldBytes, &cgrEv.Event); err != nil {
					return err
				}
			...
			case utils.Tenant:
				if err := rs.ms.Unmarshal(fieldBytes, &cdrTenant); err != nil {
					return err
				}
			}
		}
		...
		for indexKey := range idx {
			if err := rs.Cmd(nil, redisSREM, utils.CDRsIndexes+
				utils.ConcatenatedKey(cdrTenant, indexKey), key); err != nil {
				return err
			}
		}
