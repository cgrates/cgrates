...
var (
	...
	sTestsOnStorIT = []func(t *testing.T){
		...
		testOnStorITCRUDAccountActionPlans2,
...
func TestOnStorIT(t *testing.T) {
	...
	case utils.MetaInternal:
		idb, err := NewInternalDB(nil, nil, true, false, cfg.DataDbCfg().Items)
		if err != nil {
			t.Fatal(err)
		}
		onStor = NewDataManager(idb, config.CgrConfig().CacheCfg(), nil)
	...
	case utils.MetaMongo:
		...
		if mgoITdb, err = NewMongoStorage("mongodb", mgoITCfg.StorDbCfg().Host,
...
func testOnStorITCacheActionTriggers(t *testing.T) {
	...
	atsID := ats[0].ID
	if err := onStor.SetActionTriggers(atsID, ats); err != nil {
		t.Error(err)
	}
...
func testOnStorITRatingProfile(t *testing.T) {
	...
	if err = onStor.RemoveRatingProfile(rpf.Id); err != nil {
		t.Error(err)
	}
	//check database if removed
...
func testOnStorITActions(t *testing.T) {
	...
	if err := onStor.RemoveActions(acts[0].Id); err != nil {
		t.Error(err)
	}
	//check database if removed
	if _, rcvErr := onStor.GetActions(acts[0].Id,
...
func testOnStorITCRUDAccountActionPlans(t *testing.T) {
	...
	if v, rcvErr := onStor.GetAccountActionPlans(acntID, false, true, utils.NonTransactional); rcvErr != utils.ErrNotFound {
		t.Error(rcvErr, v)
	}
}

func testOnStorITCRUDAccountActionPlans2(t *testing.T) {
	acntID := utils.ConcatenatedKey("cgrates.org2", "1001")
	acntID2 := utils.ConcatenatedKey("cgrates.org2", "1002")
	expect := []string{"PACKAGE_10_SHARED_A_15", "PACKAGE_10_SHARED_A_5"}
	// aAPs := []string{"PACKAGE_10_SHARED_A_5"}
	if err := onStor.SetActionPlan("PACKAGE_10_SHARED_A_5", &ActionPlan{
		Id:            "PACKAGE_10_SHARED_A_5",
		AccountIDs:    utils.NewStringMap(acntID, acntID2),
		ActionTimings: []*ActionTiming{{}},
	}, true, utils.NonTransactional); err != nil {
		t.Error(err)
	}
	if err := onStor.SetActionPlan("PACKAGE_10_SHARED_A_15", &ActionPlan{
		Id:            "PACKAGE_10_SHARED_A_15",
		AccountIDs:    utils.NewStringMap(acntID, acntID2),
		ActionTimings: []*ActionTiming{{}},
	}, true, utils.NonTransactional); err != nil {
		t.Error(err)
	}
	Cache.SetWithoutReplicate(utils.CacheAccountActionPlans, acntID2, nil, nil, true, utils.NonTransactional)
...
func testOnStorITResourceProfile(t *testing.T) {
	...
	if err := onStor.RemoveResourceProfile(rL.Tenant, rL.ID, false); err != nil {
		t.Error(err)
	}
	//check database if removed
	if _, rcvErr := onStor.GetResourceProfile(rL.Tenant, rL.ID,
...
func testOnStorITResource(t *testing.T) {
	...
	if err := onStor.RemoveResource(res.Tenant, res.ID); err != nil {
		t.Error(err)
	}
	//check database if removed
	if _, rcvErr := onStor.GetResource(res.Tenant, res.ID,
...
func testOnStorITStatQueue(t *testing.T) {
	...
	if err := onStor.RemoveStatQueue(sq.Tenant, sq.ID); err != nil {
		t.Error(err)
	}
	//check database if removed
...
func testOnStorITThreshold(t *testing.T) {
	...
	if err := onStor.RemoveThreshold(th.Tenant, th.ID); err != nil {
		t.Error(err)
	}
	//check database if removed
...
func testOnStorITFilter(t *testing.T) {
	...
	if err := onStor.RemoveFilter(fp.Tenant, fp.ID, true); err != nil {
		t.Error(err)
	}
	//check cache if removed
	if _, rcvErr := onStor.GetFilter("cgrates.org", "Filter1",
