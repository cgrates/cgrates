...
func testOnStorITIsDBEmpty(t *testing.T) {
	test, err := onStor.DB()[utils.MetaDefault].IsDBEmpty()
	if err != nil {
		t.Error(err)
	} else if test != true {
...
func testOnStorITResourceProfile(t *testing.T) {
	...
	if itm, err := onStor.DB()[utils.MetaDefault].GetKeysForPrefix(context.TODO(), utils.ResourceProfilesPrefix, ""); err != nil {
		t.Error(err)
...
func testOnStorITResource(t *testing.T) {
	...
	if itm, err := onStor.DB()[utils.MetaDefault].GetKeysForPrefix(context.TODO(), utils.ResourcesPrefix, ""); err != nil {
		t.Error(err)
...
func testOnStorITCRUDStructVersion(t *testing.T) {
	if _, err := onStor.DB()[utils.MetaDefault].GetVersions(utils.AccountsStr); err != utils.ErrNotFound {
		t.Error(err)
	}
	vrs := Versions{
		utils.AccountsStr: 3,
	...
	if rcv, err := onStor.DB()[utils.MetaDefault].GetVersions(""); err != nil {
		t.Error(err)
	} else if !reflect.DeepEqual(vrs, rcv) {
		t.Errorf("Expecting: %v, received: %v", vrs, rcv)
	}
	delete(vrs, utils.Actions)
	if err := onStor.DB()[utils.MetaDefault].SetVersions(vrs, true); err != nil { // overwrite
		t.Error(err)
	}
	if rcv, err := onStor.DB()[utils.MetaDefault].GetVersions(""); err != nil {
		t.Error(err)
	...
	if rcv, err := onStor.DB()[utils.MetaDefault].GetVersions(utils.AccountsStr); err != nil { //query one element
		t.Error(err)
	} else if !reflect.DeepEqual(eAcnts, rcv) {
		t.Errorf("Expecting: %v, received: %v", eAcnts, rcv)
	}
	if _, err := onStor.DB()[utils.MetaDefault].GetVersions(utils.NotAvailable); err != utils.ErrNotFound { //query non-existent
		t.Error(err)
	}
	eAcnts[utils.AccountsStr] = 2
	vrs[utils.AccountsStr] = eAcnts[utils.AccountsStr]
	if err := onStor.DB()[utils.MetaDefault].SetVersions(eAcnts, false); err != nil { // change one element
		t.Error(err)
	}
	if rcv, err := onStor.DB()[utils.MetaDefault].GetVersions(""); err != nil {
		t.Error(err)
	...
	if err := onStor.DB()[utils.MetaDefault].RemoveVersions(eAcnts); err != nil { // remove one element
		t.Error(err)
	}
	...
	if err := onStor.DB()[utils.MetaDefault].RemoveVersions(nil); err != nil { // remove one element
		t.Error(err)
	}
	if _, err := onStor.DB()[utils.MetaDefault].GetVersions(""); err != utils.ErrNotFound { //query non-existent
		t.Error(err)
	}
}
...
func testOnStorITStatQueueProfile(t *testing.T) {
	...
	if itm, err := onStor.DB()[utils.MetaDefault].GetKeysForPrefix(context.TODO(), utils.StatQueueProfilePrefix, ""); err != nil {
		t.Error(err)
	} else if !reflect.DeepEqual(expectedR, itm) {
		t.Errorf("Expected : %+v, but received %+v", expectedR, itm)
	}
...
func testOnStorITStatQueue(t *testing.T) {
	...
	if itm, err := onStor.DB()[utils.MetaDefault].GetKeysForPrefix(context.TODO(), utils.StatQueuePrefix, ""); err != nil {
		t.Error(err)
	} else if !reflect.DeepEqual(expectedT, itm) {
		t.Errorf("Expected : %+v, but received %+v", expectedT, itm)
	}
...
func testOnStorITThresholdProfile(t *testing.T) {
	...
	if itm, err := onStor.DB()[utils.MetaDefault].GetKeysForPrefix(context.TODO(), utils.ThresholdProfilePrefix, ""); err != nil {
		t.Error(err)
	} else if !reflect.DeepEqual(expectedR, itm) {
		t.Errorf("Expected : %+v, but received %+v", expectedR, itm)
	}
	//update
	th.ActionProfileIDs = []string{"Action1", "Action2"}
	if err := onStor.SetThresholdProfile(context.TODO(), th, true); err != nil {
		t.Error(err)
	}
...
func testOnStorITThreshold(t *testing.T) {
	...
	if itm, err := onStor.DB()[utils.MetaDefault].GetKeysForPrefix(context.TODO(), utils.ThresholdPrefix, ""); err != nil {
		t.Error(err)
	} else if !reflect.DeepEqual(expectedT, itm) {
		t.Errorf("Expected : %+v, but received %+v", expectedT, itm)
	}
...
func testOnStorITFilter(t *testing.T) {
	...
	if itm, err := onStor.DB()[utils.MetaDefault].GetKeysForPrefix(context.TODO(), utils.FilterPrefix, ""); err != nil {
		t.Error(err)
...
func testOnStorITRouteProfile(t *testing.T) {
	...
	if err := onStor.SetRouteProfile(context.Background(), splProfile, false); err != nil {
		t.Error(err)
	}
	//get from cache
	...
	if itm, err := onStor.DB()[utils.MetaDefault].GetKeysForPrefix(context.TODO(), utils.RouteProfilePrefix, ""); err != nil {
		t.Error(err)
	} else if !reflect.DeepEqual(expectedT, itm) {
		t.Errorf("Expected : %+v, but received %+v", expectedT, itm)
	}
...
func testOnStorITAttributeProfile(t *testing.T) {
	...
	if itm, err := onStor.DB()[utils.MetaDefault].GetKeysForPrefix(context.TODO(), utils.AttributeProfilePrefix, ""); err != nil {
		t.Error(err)
	} else if !reflect.DeepEqual(expectedT, itm) {
		t.Errorf("Expected : %+v, but received %+v", expectedT, itm)
	}
...
func testOnStorITChargerProfile(t *testing.T) {
	...
	if itm, err := onStor.DB()[utils.MetaDefault].GetKeysForPrefix(context.TODO(), utils.ChargerProfilePrefix, ""); err != nil {
		t.Error(err)
	} else if !reflect.DeepEqual(expectedT, itm) {
		t.Errorf("Expected : %+v, but received %+v", expectedT, itm)
	}
...
func testOnStorITRateProfile(t *testing.T) {
	...
	if itm, err := onStor.DB()[utils.MetaDefault].GetKeysForPrefix(context.TODO(), utils.RateProfilePrefix, ""); err != nil {
		t.Error(err)
	} else if !reflect.DeepEqual(expectedT, itm) {
		t.Errorf("Expected : %+v, but received %+v", expectedT, itm)
	}
...
func testOnStorITActionProfile(t *testing.T) {
	...
	if rcv, err := onStor.DB()[utils.MetaDefault].GetKeysForPrefix(context.TODO(), utils.ActionProfilePrefix, ""); err != nil {
		t.Error(err)
...
func testOnStorITAccount(t *testing.T) {
	...
	if rcv, err := onStor.DB()[utils.MetaDefault].GetKeysForPrefix(context.TODO(), utils.AccountPrefix, ""); err != nil {
		t.Error(err)
	} else if !reflect.DeepEqual(expectedKey, rcv) {
		t.Errorf("Expecting: %v, received: %v", expectedKey, rcv)
	}
