...
func NewAMQPER(cfg *config.CGRConfig, cfgIdx int, eventChan, partialEvChan chan *erEvent, errChan chan error,
	...
	rdr.createClient(rdr.Config().Opts, exitChan)
	return rdr, nil
}
...
func (rdr *AMQPER) createClient(opts *config.EventReaderOpts, exitChan chan struct{}) {
	rdrCfg := rdr.Config()
	...
	if opts != nil {
		if opts.AMQPQueueID != nil {
			rdr.client.queueID = *opts.AMQPQueueID
		}
		if opts.AMQPConsumerTag != nil {
			rdr.client.consumer = *opts.AMQPConsumerTag
		}
		if opts.AMQPRoutingKey != nil {
			rdr.client.routingKey = *opts.AMQPRoutingKey
		}
		if opts.AMQPExchange != nil {
			rdr.client.exchange = *opts.AMQPExchange
		}
		if opts.AMQPExchangeType != nil {
			rdr.client.exchangeType = *opts.AMQPExchangeType
		}
	}
