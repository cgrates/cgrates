...
func testConfigSReload(t *testing.T) {
	...
	var reply string
	if err := testRPC.Call(utils.ConfigSv1ReloadConfig, &config.ReloadArgs{
		Tenant:  "cgrates.org",
		Path:    path.Join(*dataDir, "conf", "samples", testCfgDir),
	...
	var rpl2 string
	if err := testRPC.Call(utils.ConfigSv1GetConfigAsJSON, &config.SectionWithAPIOpts{
		Tenant:  "cgrates.org",
	...
	if err := testRPC.Call(utils.ConfigSv1GetConfigAsJSON, &config.SectionWithAPIOpts{
		...
		t.Errorf("\nExpected %+v ,\n received: %+v", utils.ToIJSON(cfgStr), utils.ToIJSON(rpl6))
	}
	...
	if err := testRPC.Call(utils.ConfigSv1GetConfigAsJSON, &config.SectionWithAPIOpts{
		...
		t.Errorf("\nExpected %+v ,\n received: %+v", utils.ToIJSON(cfgStr), utils.ToIJSON(rpl9))
	}
	...
	if err := testRPC.Call(utils.ConfigSv1GetConfigAsJSON, &config.SectionWithAPIOpts{
		...
		t.Error(err)
	} else if cfgStr != rpl27 {
		t.Errorf("\nExpected %+v ,\n received: %+v", utils.ToIJSON(cfgStr), utils.ToIJSON(rpl27))
	}
	...
	if err := testRPC.Call(utils.ConfigSv1GetConfigAsJSON, &config.SectionWithAPIOpts{
		...
		t.Error(err)
	} else if cfgStr != rpl34 {
		t.Errorf("\nExpected %+v ,\n received: %+v", utils.ToIJSON(cfgStr), utils.ToIJSON(rpl34))
	}
