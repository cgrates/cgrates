...
func testSectConfigSReloadEES(t *testing.T) {
	...
	cfgStr := `{"ees":{"attributes_conns":[],"cache":{"*fileCSV":{"limit":-1,"precache":false,"remote":false,"replicate":false,"staticTTL":false,"ttl":"5s"},"*file_csv":{"limit":-1,"precache":false,"remote":false,"replicate":false,"staticTTL":false,"ttl":"5s"}},"enabled":true,"exporters":[{"attempts":1,"attributeContext":"","attributeIDs":[],"blocker":false,"concurrentRequests":0,"efsConns":["*internal"],"exportPath":"/var/spool/cgrates/ees","failedPostsDir":"/var/spool/cgrates/failed_posts","fields":[],"filters":[],"flags":[],"id":"*default","opts":{},"synchronous":false,"timezone":"","type":"*none"}]}}`
	var rpl string
	if err := testSectRPC.Call(context.Background(), utils.ConfigSv1GetConfigAsJSON, &config.SectionWithAPIOpts{
		Tenant:   "cgrates.org",
...
func testSectConfigSReloadAsteriskAgent(t *testing.T) {
	...
	if err := testSectRPC.Call(context.Background(), utils.ConfigSv1SetConfigFromJSON, &config.SetConfigFromJSONArgs{
		Tenant: "cgrates.org",
		Config: `{"asteriskAgent":{"asteriskConns":[{"address":"127.0.0.1:8088","alias":"","connectAttempts":3,"password":"CGRateS.org","reconnects":5,"user":"cgrates"}],"createCDR":false,"enabled":false,"sessions_conns":["*birpc_internal"]}}`,
	}, &reply); err != nil {
		t.Error(err)
	} else if reply != utils.OK {
		t.Errorf("Expected OK received: %+v", reply)
...
func testSectConfigSReloadFreeswitchAgent(t *testing.T) {
	...
	if err := testSectRPC.Call(context.Background(), utils.ConfigSv1SetConfigFromJSON, &config.SetConfigFromJSONArgs{
		Tenant: "cgrates.org",
		Config: `{"freeswitchAgent":{"createCDR":false,"emptyBalanceAnnFile":"","emptyBalanceContext":"","enabled":false,"eventSocketConns":[{"address":"127.0.0.1:8021","alias":"127.0.0.1:8021","password":"ClueCon","reconnects":5}],"extraFields":[],"lowBalanceAnnFile":"","maxWaitConnection":"2s","sessions_conns":["*birpc_internal"],"subscribePark":true}}`,
	}, &reply); err != nil {
		t.Error(err)
	} else if reply != utils.OK {
		t.Errorf("Expected OK received: %+v", reply)
...
func testSectConfigSReloadSIPAgent(t *testing.T) {
	...
	if err := testSectRPC.Call(context.Background(), utils.ConfigSv1SetConfigFromJSON, &config.SetConfigFromJSONArgs{
		Tenant: "cgrates.org",
		Config: `{"sip_agent":{"enabled":false,"listen":"127.0.0.1:5060","listen_net":"udp","requestProcessors":[],"retransmission_timer":"1s","sessions_conns":["*internal"],"timezone":""}}`,
	}, &reply); err != nil {
		t.Error(err)
	} else if reply != utils.OK {
		t.Errorf("Expected OK received: %+v", reply)
