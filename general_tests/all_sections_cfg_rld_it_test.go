...
func testSectConfigSReloadAnalyzer(t *testing.T) {
	...
	cfgStr := `{"analyzers":{"cleanupInterval":"1h0m0s","dbPath":"/var/spool/cgrates/analyzers","eesConns":[],"enabled":false,"indexType":"*scorch","opts":{"*exporterIDs":[]},"ttl":"24h0m0s"}}`
	var rpl string
	if err := testSectRPC.Call(context.Background(), utils.ConfigSv1GetConfigAsJSON, &config.SectionWithAPIOpts{
		Tenant:   "cgrates.org",
