...
cgrates.org,call,1001,2014-01-14T00:00:00Z,RP_ANY,`,
	...
	t.Run("Authorize", func(t *testing.T) {
		...
		if err := client.Call(utils.SessionSv1AuthorizeEvent, args, &rplyFirst); err != nil {
			t.Fatal(err)
		}
...
cgrates.org,call,1001,2014-01-14T00:00:00Z,RP_ANY,`,
	...
	t.Run("InterimUpdate30s", func(t *testing.T) {
		...
		for i := 1; i != 30; i++ {
			...
			if err := client.Call(utils.SessionSv1UpdateSession,
			...
			if err := client.Call(utils.APIerSv2GetAccount, attrs, &acnt); err != nil {
				t.Error(err)
			} else if len(acnt.BalanceMap) != 1 {
				t.Errorf("Unexpected number of balances in the account: <%v>", utils.ToJSON(acnt))
			} else if rply := acnt.BalanceMap[utils.VOICE].GetTotalValue(); rply != expAccVal {
				t.Errorf("Expected value %+v, received <%v>",
					expAccVal, utils.ToJSON(acnt.BalanceMap[utils.VOICE]))
			}
		}
	})
}
...
cgrates.org,call,1001,2014-01-14T00:00:00Z,RP_ANY,`,
	...
	t.Run("ProcessEvent", func(t *testing.T) {
		...
		expRply := &sessions.V1ProcessEventReply{MaxUsage: 0}
		var reply sessions.V1ProcessEventReply
		if err := client.Call(utils.SessionSv1ProcessEvent,
			args, &reply); err != nil {
			t.Error(err)
		...
		if err := client.Call(utils.APIerSv2GetAccount, attrs, &acnt); err != nil {
			...
			t.Errorf("Unexpected number of balances in the account: <%v>", utils.ToJSON(acnt))
		} else if rply := acnt.BalanceMap[utils.VOICE].GetTotalValue(); rply != 10000000000 {
			// it shouldnt take from the balance since its trying to take more than the balance has
			t.Errorf("Expected value %+v, received <%v>",
				10000000000, utils.ToJSON(acnt.BalanceMap[utils.VOICE]))
		}

		var cdrCnt int64
		req := &utils.RPCCDRsFilterWithArgDispatcher{RPCCDRsFilter: &utils.RPCCDRsFilter{}}
		if err := client.Call(utils.CDRsV1GetCDRsCount, req, &cdrCnt); err != nil {
		...
		if err := client.Call(utils.CDRsV1GetCDRs, argsCdr, &cdrs); err != nil {
			t.Error("Unexpected error: ", err.Error())
		} else if len(cdrs) != 1 {
...
cgrates.org,call,1001,2014-01-14T00:00:00Z,RP_ANY,`,
	...
	t.Run("ProcessEvent", func(t *testing.T) {
		...
		if err := client.Call(utils.APIerSv2GetAccount, attrs, &acnt); err != nil {
			...
			t.Errorf("Expected value %+v, received <%v>",
				0, utils.ToJSON(acnt.BalanceMap[utils.VOICE]))
		} else if acnt.BalanceMap[utils.MONETARY].GetTotalValue() != -20 {
			t.Errorf("Expected value %+v, received <%v>",
				-20, utils.ToJSON(acnt.BalanceMap[utils.MONETARY]))
		}
	...
	t.Run("InterimUpdate30s", func(t *testing.T) {
		...
		for i := 1; i != 30; i++ {
			...
			if err := client.Call(utils.APIerSv2GetAccount, attrs, &acnt); err != nil {
				...
				t.Errorf("Unexpected number of balances in the account: <%v>", utils.ToJSON(acnt))
			} else if rply := acnt.BalanceMap[utils.MONETARY].GetTotalValue(); rply != expAccVal {
				t.Errorf("Expected value %+v, received <%v>",
					expAccVal, utils.ToJSON(acnt.BalanceMap[utils.MONETARY]))
			}
		}
	})
}
...
cgrates.org,call,1001,2014-01-14T00:00:00Z,RP_ANY,`,
	...
	t.Run("ProcessEvent", func(t *testing.T) {
		...
		if err := client.Call(utils.APIerSv2GetAccount, attrs, &acnt); err != nil {
			...
			// it shouldnt take from the balance since its trying to take more than the balance has
			t.Errorf("Expected value %+v, received <%v>",
				10, utils.ToJSON(acnt.BalanceMap[utils.MONETARY]))
		}
...
cgrates.org,call,1001,2014-01-14T00:00:00Z,RP_ANY,`,
	...
	t.Run("ProcessEvent", func(t *testing.T) {
		...
		if err := client.Call(utils.APIerSv2GetAccount, attrs, &acnt); err != nil {
			...
			t.Errorf("Unexpected number of balances in the account: <%v>", utils.ToJSON(acnt))
		} else if len(acnt.BalanceMap[utils.MONETARY]) != 2 {
			t.Errorf("Unexpected number of monetary balances in the account: <%v>", utils.ToJSON(acnt))
		} else if acnt.BalanceMap[utils.MONETARY][0].GetValue() != 0 {
			t.Errorf("Expected value %+v, received <%v>",
				0, utils.ToJSON(acnt.BalanceMap[utils.MONETARY][0]))
		} else if acnt.BalanceMap[utils.MONETARY][1].GetValue() != -21 {
			t.Errorf("Expected value %+v, received <%v>",
				-21, utils.ToJSON(acnt.BalanceMap[utils.MONETARY][1]))
		}
