...
HALF2,*any,*any,*any,*any,12:00:00;23:59:59`,
	...
	t.Run("GetAccount", func(t *testing.T) {
		...
		} else if monetaryBalance.ID == "bal1" && !monetaryBalance.TimingIDs["HALF1"] {
			t.Fatalf("expected TimingIDs %v, received: %v", "HALF1", monetaryBalance.TimingIDs)
		}
		monetaryBalance = acnt.BalanceMap[utils.MONETARY][0]
		if monetaryBalance.ID != "bal2" || monetaryBalance.Value != 10 {
			t.Fatalf("received account with unexpected *monetary balance: %v", monetaryBalance)
		} else if monetaryBalance.ID == "bal2" && !monetaryBalance.TimingIDs["HALF2"] {
			t.Fatalf("expected TimingIDs %v, received: %v", "HALF2", monetaryBalance.TimingIDs)
		}
	})
	...
	t.Run("Half2ProcessExternalCDR", func(t *testing.T) {
		...
		if err := client.Call(utils.CDRsV1ProcessExternalCDR, args, &reply); err != nil {
			t.Error(err)
	...
	t.Run("Half1GetAccount", func(t *testing.T) {
		...
		if monetaryBalance.ID != "bal1" || monetaryBalance.Value != 9.4 {
			t.Fatalf("received account with unexpected *monetary balance: %v", utils.ToJSON(monetaryBalance))
		} else if monetaryBalance.ID == "bal1" && !monetaryBalance.TimingIDs["HALF1"] {
			t.Fatalf("expected TimingIDs %v, received: %v", "HALF1", monetaryBalance.TimingIDs)
		}
		monetaryBalance = acnt.BalanceMap[utils.MONETARY][0]
