...
func TestCosts1SetStorage(t *testing.T) {
	...
	dataDBInt, err := engine.NewInternalDB(nil, nil, true, false, config.CgrConfig().DataDbCfg().Items)
	if err != nil {
		t.Error(err)
	}
...
cgrates.org,sms,*any,2012-01-01T00:00:00Z,RP_SMS1,`
	...
	if cachedRProfiles := len(engine.Cache.GetItemIDs(utils.CacheRatingProfiles, utils.EmptyString)); cachedRProfiles != 3 {
