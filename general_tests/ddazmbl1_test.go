...
cgrates.org,call,discounted_minutes,2013-01-06T00:00:00Z,RP_UK_Mobile_BIG5_PKG,`
	sharedGroups := ``
	actions := `TOPUP10_AC,*topup_reset,,,,*monetary,,*any,,,*unlimited,,10,10,false,false,10
TOPUP10_AC1,*topup_reset,,,,*voice,,DST_UK_Mobile_BIG5,discounted_minutes,,*unlimited,,40000000000,10,false,false,10`
...
TOPUP10_AT,TOPUP10_AC1,ASAP,10`
	...
	csvr, err := engine.NewTpReader(dataDB.DataDB(),
		engine.NewStringCSVStorage(utils.CSVSep, destinations, timings, rates,
			...
			attrProfiles, chargerProfiles, ``, "", utils.EmptyString, utils.EmptyString, utils.EmptyString), "", "", nil, nil, false)
	if err != nil {
		t.Error(err)
	}
