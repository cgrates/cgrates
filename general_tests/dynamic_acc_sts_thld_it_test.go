			...
			Actions: []*utils.TPAction{
				{
					...
					// get tenant and accountID from event, threshold triggers when the event account matches the already dynamicaly created account. If it matches the filter, block other thresholds from matching with the event. Make sure dynamic thresholds weight is higher than the initiative threshold THD_DYNAMIC_STATS_AND_THRESHOLD_INIT
				...
				},
				{
					...
					ExtraParameters: "*tenant;THD_ACNT_<~*req.Account>;*string:~*req.*sum#1:100;*now;-1;1;24h;true;4;ACT_BLOCK_ACC;true;",
				},
				{
					Identifier: utils.MetaDynamicStats,
					// get tenant and accountID from event, stat triggers when an event contains account with dynamicaly created accountID and also has a RequestType field in the event, each encounter that matches the filters will raise the *sum number and call the thresholdIDs specified. when the ttl is reached, *sum will go down also
					ExtraParameters: "*tenant;Stat_<~*req.Account>;*string:~*req.Account:<~*req.Account>&*exists:~*req.RequestType:;*now;;5s;;*sum#1;;false;false;10;THD_ACNT_<~*req.Account>&THD_BLOCKER_ACNT_<~*req.Account>;",
				},
			},
		}
		var reply string
