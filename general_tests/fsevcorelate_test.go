...
func TestEvCorelate(t *testing.T) {
	...
	cdrEv, err := new(cdrs.FSCdr).New(jsonCdr)
	if err != nil {
		t.Errorf("Error loading cdr: %v", err.Error())
	} else if acntId := cdrEv.GetAccId(); acntId != "86cfd6e2-dbda-45a3-b59d-f683ec368e8b" {
		t.Error("Unexpected acntId received", acntId)
	}
	if answerEv.GetCgrId() != cdrEv.GetCgrId() {
		t.Error("CgrIds do not match", answerEv.GetCgrId(), cdrEv.GetCgrId())
	}
