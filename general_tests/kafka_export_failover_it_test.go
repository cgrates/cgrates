...
"ees": {
	...
	t.Run("replay failed posts", func(t *testing.T) {
		t.Skip("EfSv1.ReplayEvents has a gob decoding bug on 1.0, see efs/efs_it_test.go")
		...
		var reply string
		if err := client.Call(context.Background(), utils.EfSv1ReplayEvents,
			efs.ReplayEventsParams{
				Tenant:     "cgrates.org",
				Provider:   utils.EEs,
	...
	t.Run("new exports after recovery arrive", func(t *testing.T) {
		t.Skip("expected record count depends on the skipped replay subtest")
		for i := 1; i <= 3; i++ {
			var err error
