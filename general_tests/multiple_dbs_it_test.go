...
func TestMultipleDBs(t *testing.T) {
	...
	t.Run("CountDBFiles", func(t *testing.T) {
		var dirs, files int
		if err := filepath.WalkDir(cfg.DbCfg().DBConns["intrnl"].Opts.InternalDBDumpPath, func(root string, info fs.DirEntry, err error) error {
			if err != nil {
				return err
			}
			if info.IsDir() {
...
func TestMultipleDBsMongo(t *testing.T) {
	...
	t.Run("CountDBFiles", func(t *testing.T) {
		var dirs, files int
		if err := filepath.WalkDir(cfg.DbCfg().DBConns["internalDBConn"].Opts.InternalDBDumpPath, func(root string, info fs.DirEntry, err error) error {
			if err != nil {
				return err
			}
			if info.IsDir() {
