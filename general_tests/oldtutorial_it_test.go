...
// 	tutLocalCfgPath = path.Join(**utils.DataDir, "conf", "samples", "tutmysql")
// 	// Init config first
// 	var err error
...
// 		t.Error(err)
// 	}
// 	tutFsLocalCfg.DataFolderPath = **utils.DataDir // Share DataFolderPath through config towards StoreDb for Flush()
...
// 	if _, err := engine.StopStartEngine(tutLocalCfgPath, **utils.WaitRater); err != nil {
// 		t.Fatal(err)
// 	}
// }
...
// 		t.Error(err)
// 	}
// 	time.Sleep(100*time.Millisecond + time.Duration(**utils.WaitRater)*time.Millisecond) // Give time for scheduler to execute topups
// }
...
// 		t.Error("Unexpected reply received: ", reply)
// 	}
// 	time.Sleep(time.Duration(**utils.WaitRater) * time.Millisecond) // Give time for CDR to be processed
