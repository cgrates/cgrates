			...
			Routes: []*routes.SortedRoute{
				{
					...
					SortingData: map[string]any{
						utils.Cost:       nil,
						utils.AccountIDs: []any{"ACCNT_ROUTES2"},
						utils.Weight:     55.,
					},
				},
				{
					...
					SortingData: map[string]any{
						utils.Cost:       5.,
						utils.AccountIDs: []any{"ACCNT_ROUTES1"},
						utils.Weight:     20.,
					},
				},
				{
...
func TestRoutesCaseV1IT(t *testing.T) {
	...
	case utils.MetaMongo:
		rtsCaseSv1ConfDIR = "routes_cases_mongo"
...
func testV1RtsCaseRpcConn(t *testing.T) {
	rtsCaseSv1BiRpc = engine.NewRPCClient(t, rtsCaseSv1Cfg.ListenCfg(), *utils.Encoding)
}
...
func testV1RtsCaseGetRoutesAfterLoading(t *testing.T) {
	...
	expRt1 := &utils.RouteProfile{
		...
		Routes: []*utils.Route{
			...
			{
				ID:        "vendor2",
				FilterIDs: []string{"*gte:~*accounts.1001.Balances[Concrete1].Units:10"},
				Weights: utils.DynamicWeights{
					{
						Weight: 20,
					},
	...
	expRt2 := &utils.RouteProfile{
		...
		Routes: []*utils.Route{
			...
			{
				ID:             "vendor4",
				FilterIDs:      []string{"*ai:~*opts.*startTime:2013-06-01T00:00:00Z|2013-06-01T10:00:00Z"},
	...
	expRt5 := &utils.RouteProfile{
		...
		Routes: []*utils.Route{
			...
			{
				ID:      "vendor3",
				StatIDs: []string{"STATS_VENDOR_2:*distinct#~*opts.*usage"},
			},
		},
	}
	if err := rtsCaseSv1BiRpc.Call(context.Background(), utils.AdminSv1GetRouteProfile,
...
func testV1RtsCasesSortingRoutesWeightNotMatchingValue(t *testing.T) {
	...
	argsBal := &utils.ArgsActSetBalance{
		Tenant:    "cgrates.org",
		AccountID: "1001",
		Diktats: []*utils.BalDiktat{
			{
				Path:  "*balance.Concrete1.Units",
				Value: "5",
			},
		},
		Reset: true,
...
func testV1RtsCasesSortingRoutesLCSetStatsAndResForMatching(t *testing.T) {
	...
	var reply string
	if err := rtsCaseSv1BiRpc.Call(context.Background(), utils.ResourceSv1AllocateResources,
		ev, &reply); err != nil {
		t.Error(err)
...
func testV1RtsCasesRoutesProcessStatsForLoadRtsSorting(t *testing.T) {
	...
	if err := rtsCaseSv1BiRpc.Call(context.Background(), utils.StatSv1ProcessEvent, ev1, &reply); err != nil {
		t.Error(err)
	} else if err := rtsCaseSv1BiRpc.Call(context.Background(), utils.StatSv1ProcessEvent, ev1, &reply); err != nil {
		t.Error(err)
	} else {
		sort.Strings(expected)
		sort.Strings(reply)
...
func testV1RtsCasesSortRoutesHigherCostAllocateRes(t *testing.T) {
	...
	if err := rtsCaseSv1BiRpc.Call(context.Background(), utils.ResourceSv1ReleaseResources,
		evRs, &result); err != nil {
		t.Error(err)
	} else if result != utils.OK {
		t.Errorf("Unexpected result returned: %s", result)
