...
var (
	...
	SessionsBkupIntrvlTests = []func(t *testing.T){
		...
		testSessionSBkupIntrvlGetBackedupSessions3,
		testSessionSBkupIntrvlStopCgrEngine,
...
cgrates.org,call,*any,2014-01-14T00:00:00Z,RP_ANY,`,
	...
	customTpPath := "/tmp/testTPs/BackupTPs"
	if err := loadCSVs(sBkupRPC, "", customTpPath, tpFiles); err != nil {
		t.Fatal(err)
	}
...
func testSessionSBkupIntrvlGetBackedupSessions1(t *testing.T) {
	...
	var getBackSess []*sessions.Session
	if sessByte, err := dDB.GetBackedupSessionsDrv(sBkupCfg.GeneralCfg().NodeID,
		sBkupCfg.GeneralCfg().DefaultTenant); err != nil {
		t.Error(err)
	} else if sessByte != nil {

		ms := engine.NewCodecMsgpackMarshaler()
		if err = ms.Unmarshal(sessByte, &getBackSess); err != nil {
			t.Error(err)
		}
	}

	if len(getBackSess) != 500 {
		t.Fatalf("Expected 500 sessions stored, received %v", len(getBackSess))
	}
	for _, oneSess := range getBackSess {
		...
		if !found {
			t.Errorf("Session not found: %+v", oneSess)
		}
	}
...
func testSessionSBkupIntrvlGetBackedupSessions2(t *testing.T) {
	...
	if len(getBackSess) == 0 { //since it takes more then 500ms(backup_interval) to terminate 500 sessions, it backed up midway through them getting terminated, so it doesnt know all of them terminated yet
		t.Fatalf("Expected sessions stored, received %v", len(getBackSess))
	}
	time.Sleep(500 * time.Millisecond) // wait for backup_interval to backup again now that
}

func testSessionSBkupIntrvlGetBackedupSessions3(t *testing.T) {
	time.Sleep(500 * time.Millisecond) // wait for backup_interval to backup again
	var getBackSess []*sessions.Session
	if sessByte, err := dDB.GetBackedupSessionsDrv(sBkupCfg.GeneralCfg().NodeID,
		sBkupCfg.GeneralCfg().DefaultTenant); err != nil {
		t.Error(err)
	...
	if len(getBackSess) != 0 { // by now it backed up again so there shoule be 0 sessions saved
		t.Fatalf("Expected 0 sessions stored, received %v", len(getBackSess))
	}
}
