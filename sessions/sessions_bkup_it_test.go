...
func testSessionSBkupInitiate(t *testing.T) {
	...
	// Delay further 4 seconds to make the session unrestorable (backup_entry_ttl is set 4s in configs)
...
func testSessionSBkupGetBackedupSessions(t *testing.T) {
	...
	if sessByte, err := dataDB.GetBackedupSessionsDrv(sBkupCfg.GeneralCfg().NodeID,
