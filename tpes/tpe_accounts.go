...
func (tpAcc TPAccounts) exportItems(ctx *context.Context, wrtr io.Writer, tnt string, itmIDs []string) (err error) {
	if len(itmIDs) == 0 {
		...
		// dbKeys will contain the full name of the key, but we will need just the IDs e.g. "acn_cgrates.org:ACC_1" -- just ACC_1
		var dbKeys []string
		if dbKeys, err = tpAcc.dm.DataDB().GetKeysForPrefix(ctx, prfx); err != nil {
			return err
		}
		...
		// the map e.g. : *filters: {"ACC_1", "ACC_2"}
		itmIDs = profileIDs
	}
