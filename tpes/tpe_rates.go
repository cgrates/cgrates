...
func (tpRts TPRates) exportItems(ctx *context.Context, wrtr io.Writer, tnt string, itmIDs []string) (err error) {
	if len(itmIDs) == 0 {
		...
		// dbKeys will contain the full name of the key, but we will need just the IDs e.g. "acn_cgrates.org:RATE_1" -- just RATE_1
		var dbKeys []string
		if dbKeys, err = tpRts.dm.DataDB().GetKeysForPrefix(ctx, prfx); err != nil {
			return err
		}
		...
		// the map e.g. : *filters: {"RATE_1", "RATE_1"}
		itmIDs = profileIDs
	}
