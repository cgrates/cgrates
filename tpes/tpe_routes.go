...
func (tpRoutes TPRoutes) exportItems(ctx *context.Context, wrtr io.Writer, tnt string, itmIDs []string) (err error) {
	if len(itmIDs) == 0 {
		...
		// dbKeys will contain the full name of the key, but we will need just the IDs e.g. "acn_cgrates.org:ROUTE_1" -- just ROUTE_1
		var dbKeys []string
		if dbKeys, err = tpRoutes.dm.DataDB().GetKeysForPrefix(ctx, prfx); err != nil {
			return err
		}
		...
		// the map e.g. : *filters: {"ROUTE_1", "ROUTE_1"}
		itmIDs = profileIDs
	}
	csvWriter := csv.NewWriter(wrtr)
	csvWriter.Comma = utils.CSVSep
	// before writing the profiles, we must write the headers
	if err = csvWriter.Write([]string{"#Tenant", "ID", "FilterIDs", "Weights", "Sorting", "SortingParameters", "RouteID", "RouteFilterIDs", "RouteAccountIDs", "RouteRateProfileIDs", "RouteResourceIDs", "RouteStatIDs", "RouteWeights", "RouteBlocker", "RouteParameters"}); err != nil {
		return
	}
