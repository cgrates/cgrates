...
//   - 'side' specifies the side of the string to strip, and it must be '*prefix', '*suffix'
//     or '*both'.
//   - 'substr' is the substring to be removed.
//   - 'amount' (optional) is the number of times 'substr' should be removed.
//
// If 'amount' is omitted, it defaults to -1, which will remove all instances of substr from the chosen
// side. If 'substr' is empty, 'amount' characters will be removed from the specified side
// regardless of their value.
func NewStripConverter(params string) (DataConverter, error) {
	paramSlice := strings.Split(params, InInFieldSep)
	if len(paramSlice) < 3 || len(paramSlice) > 4 {
	...
	if sc.substr == EmptyString && sc.amount == -1 {
