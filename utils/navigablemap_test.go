...
func TestNavigableMap2SetField(t *testing.T) {
	...
	expected := NavigableMap2{"Field1": NewNMData(10)}
	...
	expected = NavigableMap2{
		"Field1": NewNMData(10),
		"Field2": &NMSlice{NewNMData("1002")},
	}
	if err := nm.SetField(PathItem{Field: "Field2", Index: IntPointer(10)}, NewNMData("1002")); err != ErrWrongPath {
		t.Error(err)
	}
	if err := nm.SetField(PathItem{Field: "Field2", Index: IntPointer(0)}, NewNMData("1002")); err != nil {
		t.Error(err)
	} else if !reflect.DeepEqual(nm, expected) {
		t.Errorf("Expected %s ,received: %s", expected, nm)
	}
	if err := nm.SetField(PathItem{Field: "Field1", Index: IntPointer(0)}, NewNMData(10)); err != ErrWrongPath {
		t.Error(err)
	}
	expected = NavigableMap2{
		"Field1": NewNMData("1001"),
	...
	if err := nm.SetField(PathItem{Field: "Field1"}, NewNMData("1001")); err != nil {
		t.Error(err)
	} else if !reflect.DeepEqual(nm, expected) {
		t.Errorf("Expected %s ,received: %s", expected, nm)
	}
	expected = NavigableMap2{
		"Field1": NewNMData("1001"),
		"Field2": &NMSlice{NewNMData("1003")},
	}
	if err := nm.SetField(PathItem{Field: "Field2", Index: IntPointer(0)}, NewNMData("1003")); err != nil {
		t.Error(err)
	} else if !reflect.DeepEqual(nm, expected) {
		t.Errorf("Expected %s ,received: %s", expected, nm)
	}
